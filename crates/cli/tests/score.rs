...
fn builds_and_runs_score_component() {
    ...
    let wasm = std::env::temp_dir().join("starstream-run-score-cli-test.wasm");
    std::fs::write(&wasm, EXAMPLE_SCORE).expect("failed to write the score guest to a temp file");
